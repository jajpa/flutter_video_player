...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  Future<void> _applyQuality() async {
    ...
    await _videoPlayerPlatform.setQuality(_textureId, value.currentQuality);
  }
  ...
  Future<void> _getQualities() async {
    ...
    var qualities = await _videoPlayerPlatform.qualities(_textureId);
    print('qualities 11111');
    print(qualities);
    qualities.removeWhere((val) => val == -1);
    value = value.copyWith(qualities: qualities);
    return;
  }

  Future<void> _getCurrentQuality() async {
    if (!value.initialized || _isDisposed) {
      return;
    }
    var result = await _videoPlayerPlatform.currentQuality(_textureId);
    value = value.copyWith(currentQuality: result);
    return;
  }
  ...
  Future<void> setQuality(int quality) async {
    value = value.copyWith(currentQuality: quality);
    await _applyQuality();
  }
